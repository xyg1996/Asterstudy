...
"""
    ...
    assert_that(stage1.model, not_none())

    #--------------------------------------------------------------------------
    stage2 = case.create_stage(':2:')
    assert_that(stage2, is_in(case))
...
"""
    ...
    assert_that(stage2.model, not_none())

    #--------------------------------------------------------------------------
    command = stage2['MODELUPG']
    assert_that(command, is_in(stage2))
    ...
    assert_that(stage1.model, none())
    assert_that(stage1, is_not(is_in(case)))

    assert_that(stage2.model, none())
    assert_that(stage2, is_not(is_in(case)))
    assert_that(command, is_not(is_in(stage2)))
    assert(check_export(stage2, "\n\n", validity=False))

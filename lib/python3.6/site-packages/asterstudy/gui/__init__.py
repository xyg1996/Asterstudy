...
from ..datamodel import CATA, ConversionLevel
...
class WorkingMode:
    ...
    Attributes:
        ...
        ResultsMode: *Results* working mode.
    """
    ...
    @staticmethod #静态方法，无需实例化即可调用装饰的方法
...
class Panel: #面板
    ...
    Attributes:
        Data: *Data* panel. 数据面板
        View: *View* panel. 显示面板
        Edit: *Edition* panel. 编辑面板
    """
...
class ResourceDir:
    ...
    Attributes:
        Assistant: Calculation assistants directory. # 计算相关协同路径
        Templates: Templates directory. # 模板路径
    """
    Assistant = 0
    Templates = 1
...
def translate_command(command, keyword=None, item=None,
    ...
    return translate("Commands", CATA.get_translation(command, keyword, item))
...
def is_noname(command):
    """
    Check if command is 'noname' (does not produce concept).

    Arguments:
        command (Command): Command object.

    Returns:
        bool: *True* if command is 'noname'; *False* otherwise.
    """
    return command.gettype(ConversionLevel.NoFail) is None
...
def clipboard_text():
    ...
    if clipboard.mimeData(Q.QClipboard.Clipboard) and clipboard.mimeData(Q.QClipboard.Clipboard).hasText():
        text = clipboard.mimeData(Q.QClipboard.Clipboard).text()
    elif clipboard.mimeData(Q.QClipboard.Selection) and clipboard.mimeData(Q.QClipboard.Selection).hasText() \

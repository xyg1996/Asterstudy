...
def views():
    """
    Get all views managed by Workspace.
...
class Workspace(Q.QWidget):
    ...
    def __init__(self, astergui, parent=None, tab_position=Q.QTabWidget.West):#标签页位置设置
        ...
        v_layout = Q.QVBoxLayout(self.pages[WorkingMode.CaseMode]) # 一级垂直排布
        v_layout.setContentsMargins(2, 3, 2, 3)
        v_layout.setSpacing(3)

        # C.1. Add "read-only" banner to the top-most area of page # 设置一些按钮不可用
        ...
        # C.2. Add top-level horizontal splitter 二级水平分割
        ...
        # C.3. Add vertical splitter to horizontal splitter (split left area vertically) 三级左侧部分垂直分割3份
        ...
        # C.7. Set-up "View" panel within horizontal splitter (central area) # 二级添加mesh显示面板
        ...
        # C.8. Set-up "Edition" panel (right area) # 二级添加编辑面板

        self.panels[Panel.Edit] = EditionPanel(self.case_splitter) # 打开时默认隐藏，见editionpanel.py self.hide()
        ...
        connect(self.main.currentChanged, self.modeChanged) # void QTabWidget::currentChanged(int index) / 信号与信号的连接 / 当标签页改变时触发
        connect(self.main.currentChanged, self._updateBanner)
        connect(self.main.currentChanged, self._selectActiveCase)
        connect(self.panels[Panel.Edit].editorClosed, self.views[Context.Information].update) # 编辑完毕关闭时，触发更新Information里的内容

        for view in self.views.values():
            view.setContextMenuPolicy(Q.Qt.CustomContextMenu)
            connect(view.customContextMenuRequested, self._popupMenuRequest)
            if hasattr(view, "itemSelectionChanged"): # 判断view对象是否有相应的属性功能，如双击选择item内容等
    ...
    @Q.pyqtSlot("QPoint") #使用装饰的槽方法，节省内存，速度更快

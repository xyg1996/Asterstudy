...
def remove_remote_dir(result, path):
    """Remove the remote database created by the given *Result*.

    Arguments:
        result (Result): Object associated to the Stage that created the
            remote database.
        path (str): Remote path of the parent Case.

    Returns:
        tuple: See `:py:func:remote_delete`.
    """
    from ..datamodel.engine import serverinfos_factory
    if not result:
        return 0, '', ''
    infos = serverinfos_factory()
    server = result.job.server
    return remote_delete(infos.server_username(server),
                         infos.server_hostname(server),
                         path)
